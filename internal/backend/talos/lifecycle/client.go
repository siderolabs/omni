...
func (m *Manager) buildInstallImage(ctx context.Context, machineID string, ms *omni.MachineStatus, version string, target *specs.MachineConfigGenOptionsSpec_InstallImage) (string, error) {
	...
	if target == nil {
		return "", fmt.Errorf("machine %q has no target install image", machineID)
	}
	...
	// resolve it from the target Talos version instead of failing the install.
	...
	return installimage.Build(machineID, target)
}
