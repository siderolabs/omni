...
// TestClientFactoryStop checks that stopping the factory evicts everything and keeps the open clients working.
...
func TestClientLeakGuard(t *testing.T) {
	...
	testutils.WithRuntime(ctx, t, testutils.TestOptions{}, func(context.Context, testutils.TestContext) {
		...
		profile := pprof.NewProfile(t.Name())
		clientFactory := talos.NewClientFactoryWithProfile(testContext.State, zap.New(core), profile)
		...
		// the leak is counted, and the leaked client stays in the profile with its creation stack
		assert.Equal(t, 1, clientFactory.LeakedClients())
...
func TestClientFactoryConcurrency(t *testing.T) {
	...
	testutils.WithRuntime(ctx, t, testutils.TestOptions{}, func(context.Context, testutils.TestContext) {
		...
		// the machines are reachable over the machine mock, so the workers do real RPCs on the shared connections
		for _, id := range machines {
			machineMock := testutils.NewMachineServiceMock(ctx, t, id, testContext.State)
			createClusterMachine(ctx, t, testContext.State, id, "alpha", machineMock.SocketConnectionString)
		}
		...
		eg.Go(func() error {
			for i := range 200 {
				clientFactory.ReleaseForMachine("alpha", machines[i%len(machines)])
				clientFactory.Sweep(time.Now().Add(talos.IdleTimeout))
			}
...
// TestClientOpenClientsProfile checks that an open client is recorded in the pprof profile and removed on Close.
func TestClientOpenClientsProfile(t *testing.T) {
	t.Parallel()
	...
	testutils.WithRuntime(ctx, t, testutils.TestOptions{}, func(context.Context, testutils.TestContext) {
		...
		profile := pprof.NewProfile(t.Name())
		clientFactory := talos.NewClientFactoryWithProfile(testContext.State, testContext.Logger, profile)
		...
		require.NoError(t, c.Close())
		require.NoError(t, c.Close()) // a second Close is a no-op, and must not remove twice

		assert.Equal(t, 0, profile.Count())
