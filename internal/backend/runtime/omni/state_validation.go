...
func joinTokenValidationOptions(st state.State) []validated.StateOption {
	validateJoinTokenName := func(res *authres.JoinToken) error {
	...
	checkDefault := func(ctx context.Context, id string) (bool, error) {
		defaultJoinToken, err := safe.ReaderGetByID[*authres.DefaultJoinToken](ctx, st, authres.DefaultJoinTokenID)
		if err != nil && !state.IsNotFoundError(err) {
			return false, err
		}
	...
	return []validated.StateOption{
		validated.WithUpdateValidations(validated.NewUpdateValidationForType(func(_ context.Context, _, res *authres.JoinToken, _ ...state.UpdateOption) error {
			return validateJoinTokenName(res)
		})),
		validated.WithCreateValidations(validated.NewCreateValidationForType(func(_ context.Context, res *authres.JoinToken, _ ...state.CreateOption) error {
		...
		validated.WithDestroyValidations(validated.NewDestroyValidationForType(
			func(ctx context.Context, _ resource.Pointer, res *authres.JoinToken, _ ...state.DestroyOption) error {
...
func defaultJoinTokenValidationOptions() []validated.StateOption {
	return []validated.StateOption{
		validated.WithUpdateValidations(validated.NewUpdateValidationForType(func(_ context.Context, _, res *authres.DefaultJoinToken, _ ...state.UpdateOption) error {
			if res.Metadata().Phase() == resource.PhaseTearingDown {
				if res.Metadata().ID() != authres.DefaultJoinTokenID {
					return nil
				}
		...
		validated.WithDestroyValidations(validated.NewDestroyValidationForType(
			func(_ context.Context, _ resource.Pointer, res *authres.DefaultJoinToken, _ ...state.DestroyOption) error {
				if res.Metadata().ID() != authres.DefaultJoinTokenID {
					return nil
				}
