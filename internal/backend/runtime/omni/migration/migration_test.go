...
func (suite *MigrationSuite) TestMachineInstallDiskConfigsFromPatches() {
	...
	// a machine whose config was already created by a previous partially-failed run (crashed between
	// the config create and the patch delete): the config must be preserved, the patch still deleted
	...
	// a patch at the well-known ID mutated to carry more than the disk: hand-written, left alone
	...
	// a patch of a template-managed cluster: the translated config must stay template-managed
	...
	// the interrupted machine: the pre-existing config is preserved, the patch is still deleted
