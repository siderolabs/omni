...
func gapError(version string) Callback { //nolint:unparam
	return func(context.Context, state.State, *zap.Logger, migrationContext) error {
		return fmt.Errorf("migration is too old, too many versions are being skipped: please upgrade to %q first", version)
	}
}
