...
// legacyInstallDiskPatchWeight is the historical weight of the machine-generated install disk
// selection patches, frozen here because the shared constant goes away with the patch mechanism.
const legacyInstallDiskPatchWeight = 0

// legacyInstallDiskFromPatch returns the disk path carried by a machine-generated install disk
// selection patch, and empty when the patch is not exactly one.
//
// Recognized means the exact generated shape: the well-known ID formed from the cluster machine
// label, and a body containing nothing but the single install disk value. A patch at the
// well-known ID that was mutated to carry anything else is treated as hand-written.
//
// This is a frozen private copy of the recognition the template export used to perform: the
// export-side code is deleted together with the patch mechanism, and migrations must not be
// coupled to living helpers.
func legacyInstallDiskFromPatch(configPatch *omni.ConfigPatch) string {
	// the machine-generated selection patches are user-owned, anything else is not one of them
...
// machineInstallDiskConfigsFromPatches translates the machine-generated install disk selection
// patches into MachineInstallDiskConfig resources and deletes them. The patches were written by
// the cluster creation UI and by cluster templates before the install disk became a first-class
// resource. Leaving one behind is not an option: the patch no longer influences the install disk
// (the verified disk is re-asserted over patches at render), so without the translation the
// user's explicit choice would silently degrade to automatic selection.
//
// Idempotent: the resource create is skipped when it already exists and the patch deletion is the
// completion marker, so a re-run after a partial failure converges.
func machineInstallDiskConfigsFromPatches(ctx context.Context, st state.State, logger *zap.Logger, _ migrationContext) error {
	...
	for configPatch := range configPatches.All() {
		...
		// A patch of a template-managed cluster was written by the template, so the config it
		// translates to must stay template-managed: without the annotation, removing the install
		// stanza from the template after the upgrade would preserve the selection as a user
