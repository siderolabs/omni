...
func (ctrl *SchematicConfigurationController) reconcile(ctx context.Context, r controller.ReaderWriter,
	...
	mc, err := ctrl.newMachineCustomization(cluster, ms, extensions, extraKernelArgs, securityState.BootedWithUki)
	if err != nil {
		return nil, nil, err
	}
...
func (ctrl *SchematicConfigurationController) newMachineCustomization(cluster *omni.Cluster, ms *omni.MachineStatus,
	exts *omni.MachineExtensions, args *omni.MachineExtraKernelArgs, isUKI bool,
	...
	if isUKI && schematicConfig == nil {
		return me, xerrors.NewTaggedf[qtransform.SkipReconcileTag]("machine is booted with UKI but the schematic information is not yet available")
	}

	me.extensions = ctrl.determineBaseExtensions(ms)
	me.kernelArgs = ctrl.determineBaseKernelArgs(ms)
	...
	if args != nil && args.Metadata().Phase() == resource.PhaseRunning {
		me.shouldGenerateSchematic = true
...
// determineBaseKernelArgs determines the base kernel args for a machine booted with UKI in a best-effort basis.
func (ctrl *SchematicConfigurationController) determineBaseKernelArgs(ms *omni.MachineStatus) []string {
	// use initial kernel args if available
	if ms.TypedSpec().Value.Schematic != nil && ms.TypedSpec().Value.Schematic.InitialState != nil {
		return ms.TypedSpec().Value.Schematic.InitialState.KernelArgs
	}

	// otherwise, use current kernel args but filter out the non-siderolink args
	return xslices.Filter(ms.TypedSpec().Value.Schematic.KernelArgs, siderolink.IsSiderolinkKernelArg)
}

// determineBaseExtensions determines the base extensions for a machine booted with UKI in a best-effort basis.
func (ctrl *SchematicConfigurationController) determineBaseExtensions(ms *omni.MachineStatus) []string {
	// use initial extensions if available
	if ms.TypedSpec().Value.Schematic != nil && ms.TypedSpec().Value.Schematic.InitialState != nil {
