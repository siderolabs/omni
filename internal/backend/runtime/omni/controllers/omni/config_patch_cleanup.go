...
func (ctrl *ConfigPatchCleanupController) Run(ctx context.Context, r controller.Runtime, logger *zap.Logger) error {
	...
	ticker := ctrl.Clock.NewTicker(ctrl.CheckInterval)
	defer ticker.Stop()
...
func (ctrl *ConfigPatchCleanupController) initDefaults() {
	...
	if ctrl.Clock == nil {
		ctrl.Clock = clockwork.NewRealClock()
	}
}
...
func (ctrl *ConfigPatchCleanupController) isOrphan(ctx context.Context, r controller.Reader, configPatch *omni.ConfigPatch) (bool, error) {
	...
	if ctrl.Clock.Now().Sub(configPatch.Metadata().Updated()) < ctrl.DeleteOlderThan {
		return false, nil
	}

	if _, isSystemPatch := configPatch.Metadata().Labels().Get(omni.LabelSystemPatch); isSystemPatch {
		return false, nil
	}
