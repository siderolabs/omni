...
// The disk is the automatic default computed from the machine's disks, or the one picked by
// the optional user selection in MachineInstallDiskConfig. ClusterMachineConfigController and
// the UI dropdowns read the resolved disk, never the inputs.
	...
	return qtransform.NewQController(
		qtransform.Settings[*omni.MachineStatus, *omni.MachineInstallDiskStatus]{
			...
			TransformFunc: func(ctx context.Context, r controller.Reader, _ *zap.Logger, machineStatus *omni.MachineStatus, status *omni.MachineInstallDiskStatus) error {
				...
				helpers.SyncLabels(machineStatus, status, omni.LabelCluster)

				resolution := Resolve(machineStatus, config)

				status.TypedSpec().Value.Disk = resolution.Disk
				status.TypedSpec().Value.Disks = resolution.Disks
