...
// TestStatusController verifies the controller wiring around Resolve: the status exists for every
// machine from its status alone, a created MachineInstallDiskConfig re-resolves it (proving the
// config is a mapped input), and deleting the config falls back to the automatic default.
	...
	testutils.WithRuntime(
		...
		func(ctx context.Context, tc testutils.TestContext) {
			...
			rtestutils.AssertResource(ctx, t, st, id, func(res *omni.MachineInstallDiskStatus, a *assert.Assertions) {
				...
				a.Equal([]string{"/dev/sda", "/dev/sdb"}, res.TypedSpec().Value.Candidates)
			})
