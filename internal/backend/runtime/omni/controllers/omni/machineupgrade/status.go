...
func (ctrl *StatusController) transform(ctx context.Context, r controller.Reader, logger *zap.Logger, ms *omni.MachineStatus, status *omni.MachineUpgradeStatus) error {
	...
	// This controller upgrades maintenance mode machines when the schematic Omni wants for them differs from the one they run.
	// The extensions and kernel args of an invalid machine are not managed, so such a difference must never be acted on.
