...
func (ctrl *SecretRotationStatusController) reconcileRunning(
	...
	_, clusterLocked := clusterStatus.Metadata().Annotations().Get(omni.ClusterLocked)

	if clusterLocked {
		logger.Warn("cluster is locked, skip reconcile", zap.String("cluster", clusterStatus.Metadata().ID()))

		return xerrors.NewTaggedf[qtransform.SkipReconcileTag]("secret rotations are not allowed: the cluster is locked")
	}
	...
	if ongoingRotations.Len() > 0 {
		return controller.NewRequeueError(fmt.Errorf("waiting for all ongoing rotations to be completed for machines: %s", xslices.Map(ongoingRotations.Candidates,
...
func (ctrl *SecretRotationStatusController) processMachine(
	ctx context.Context,
	r controller.ReaderWriter,
	machineStatus *omni.ClusterMachineStatus,
	clusterSecrets *omni.ClusterSecrets,
	rotationStatus *omni.ClusterSecretsRotationStatus,
	...
	_, isControlPlane := machineStatus.Metadata().Labels().Get(omni.LabelControlPlaneRole)
	_, locked := machineStatus.Metadata().Annotations().Get(omni.MachineLocked)
	hostname, _ := machineStatus.Metadata().Labels().Get(omni.LabelHostname)
	...
	candidate := secretrotation.Candidate{
		...
		Blocked:      locked,
		Ready:        machineStatus.TypedSpec().Value.Ready,
	}
	...
	secretRotation, ok := secretRotationsMap[machineStatus.Metadata().ID()]
	...
	if clusterSecrets.TypedSpec().Value.RotationPhase != specs.ClusterSecretsRotationStatusSpec_OK && clusterSecrets.TypedSpec().Value.RotationPhase == secretRotation.TypedSpec().Value.Phase &&
		secretRotation.TypedSpec().Value.Status == specs.ClusterMachineSecretsRotationSpec_IN_PROGRESS {
		valid, err := candidate.Validate(ctx, r, clusterSecrets)
		if err != nil {
		...
		if valid {
			if err = safe.WriterModify[*omni.ClusterMachineSecretsRotation](ctx, r, omni.NewClusterMachineSecretsRotation(secretRotation.Metadata().ID()),
				func(res *omni.ClusterMachineSecretsRotation) error {
					helpers.CopyLabels(machineStatus, res, omni.LabelCluster, omni.LabelMachineSet, omni.LabelWorkerRole, omni.LabelControlPlaneRole)
