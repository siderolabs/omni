...
func (s *Rotator) handleClusterMachineSecretRotation(
	...
	if !clusterStatus.TypedSpec().Value.Ready || clusterStatus.TypedSpec().Value.Phase != specs.ClusterStatusSpec_RUNNING {
		rotationStatus.TypedSpec().Value.Status = fmt.Sprintf("%s at stage %s", RotationPaused, rotationStatus.TypedSpec().Value.Phase.String())
...
func (s *Rotator) prepareCMSecretsForSecretRotation(
	...
	if secretRotation.TypedSpec().Value.Component == specs.SecretRotationSpec_KUBERNETES_CA && secretRotation.TypedSpec().Value.ExtraCerts.GetK8S() != nil {
