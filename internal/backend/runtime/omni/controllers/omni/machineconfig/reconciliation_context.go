...
func BuildReconciliationContext(ctx context.Context, r controller.Reader,
	...
	// The install disk comes from the config resource itself. ClusterMachineConfigController
	// records the disk on the config it generates, so the disk used for an install and the
	// config data can never disagree.
	rc.installDisk = machineConfig.TypedSpec().Value.InstallDisk

	// An empty recorded disk means the config was generated by an older Omni version and was not
	// re-generated by this one yet, so the recorded disk is not populated. In that case, machines
	// without Talos installed wait until the config is re-generated with the disk. Installed
	// machines proceed, as upgrades and config applies do not use the disk.
	if rc.installDisk == "" && omni.GetMachineStatusSystemDisk(rc.machineStatus) == "" {
		return nil, xerrors.NewTaggedf[qtransform.SkipReconcileTag](
			"%q config carries no install disk yet", machineConfig.Metadata().ID(),
		)
	}
