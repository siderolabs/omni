...
func BuildReconciliationContext(ctx context.Context, r controller.Reader,
	...
	if err != nil {
		if state.IsNotFoundError(err) {
			return nil, xerrors.NewTaggedf[qtransform.SkipReconcileTag]("'%s' machine set node found: %w", machineConfig.Metadata().ID(), err)
		}
...
func computeDiff(previousData, newData []byte) string {
