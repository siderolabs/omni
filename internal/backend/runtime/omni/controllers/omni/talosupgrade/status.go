...
//
// Each healthcheck is executed in a long-lived runner pod that is reused across evaluations and cleaned up
// once the upgrade completes (see cleanupHealthCheckRunners), so the gating doesn't pay the pod scheduling
// and image pull cost on every re-check.
