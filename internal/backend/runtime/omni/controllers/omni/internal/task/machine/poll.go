...
func pollDisks(ctx context.Context, c *client.Client, info *Info) error {
	...
	return forEachResource(
		...
		func(disk *block.Disk) error {
			...
			// Composite devices (md arrays, device-mapper volumes: SecondaryDisks non-empty or a
			// virtual bus path) are deliberately kept: install disk selectors may target them.
			// They are excluded from automatic install disk selection by the resolver instead.
			// the vendor-supplied sysfs strings are sanitized: invalid UTF-8 in any proto3 string
			// field would fail marshaling the whole machine status
