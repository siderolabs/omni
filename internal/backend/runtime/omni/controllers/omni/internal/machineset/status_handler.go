...
func ReconcileStatus(rc *ReconciliationContext, machineSetStatus *omni.MachineSetStatus) {
	...
	if isControlPlane {
		...
		spec.ConfigUpdatesAllowed = (len(rc.GetMachinesToCreate()) + len(rc.GetMachinesToTeardown()) + len(rc.GetMachinesToTeardown())) == 0
	} else {
	...
	if len(rc.GetOutdatedMachines()) > 0 {
	...
	for _, machineSetNode := range machineSetNodes {
		...
		if clusterMachineStatusSpec.Stage != specs.ClusterMachineStatusSpec_RUNNING {
			spec.Ready = false
			spec.ConfigUpdatesAllowed = false
