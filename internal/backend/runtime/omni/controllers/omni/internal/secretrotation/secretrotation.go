...
func (c Candidate) Validate(ctx context.Context, r controller.ReaderWriter, secrets *omni.ClusterSecrets) (bool, error) {
	switch secrets.TypedSpec().Value.ComponentInRotation {
	case specs.ClusterSecretsRotationStatusSpec_TALOS_CA:
		return c.validateTalosCARotation(ctx, r, secrets)
...
func (c Candidate) validateTalosCARotation(ctx context.Context, r controller.ReaderWriter, secrets *omni.ClusterSecrets) (bool, error) {
	talosClient, err := c.getTalosClient(ctx, r, secrets, c.MachineID)
	if err != nil {
		return false, err
	}
...
func (c Candidate) getTalosClient(
	...
	machineStatus, err := safe.ReaderGetByID[*omni.MachineStatus](ctx, r, machineID)
	if err != nil {
