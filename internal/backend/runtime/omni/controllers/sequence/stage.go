...
// NewInitialStage creates a new Stage with provided name and run function.
//
// The run function should return an error if any occurred during execution. A nil value indicates that the stage is completed and the controller will move to the next stage.
// If processing additional stages is not needed, an error of type ErrWait should be returned.
// There can only be one initial stage in a sequence.
func NewInitialStage[Input, Output generic.ResourceWithRD](name string, run RunFunc[Input, Output],
) Stage[Input, Output] {
	return Stage[Input, Output]{name: name, run: run, initial: true}
}
...
type Sequencer[Input, Output generic.ResourceWithRD] interface {
	...
	InitialStage() Stage[Input, Output]
	ExtraStages() []Stage[Input, Output]
}
