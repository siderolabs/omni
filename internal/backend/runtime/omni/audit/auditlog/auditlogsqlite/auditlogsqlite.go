...
func (s *Store) Write(ctx context.Context, event auditlog.Event) error {
	query := fmt.Sprintf(`INSERT INTO %s (%s, %s, %s, %s) VALUES (?, ?, ?, ?)`, tableName, eventTypeColumn, resourceTypeColumn, eventTSMillisColumn, eventDataColumn)
	...
	if _, err := s.db.ExecContext(ctx, query, event.Type, event.ResourceType, event.TimeMillis, dataJSON); err != nil {
