...
// Package auditlogfile implements a file-backed audit log store.
...
func (l *LogFile) Remove(_ context.Context, start, end time.Time) error {
	...
	dirFiles, err := getDirFiles(os.DirFS(l.dir).(fs.ReadDirFS)) //nolint:errcheck,forcetypeassert
	if err != nil {
		return err
	}
...
func (l *LogFile) reader(start, end time.Time) (io.ReadCloser, error) {
	...
	dirFiles, err := getDirFiles(os.DirFS(l.dir).(fs.ReadDirFS)) //nolint:forcetypeassert,errcheck
	if err != nil {
