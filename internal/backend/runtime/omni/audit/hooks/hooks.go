...
func Init(a *audit.Log) {
	...
	// The JWT signing keys are generated by Omni itself, and users can only destroy them,
	// to invalidate the tokens signed by a key. The update hook covers teardowns: a teardown
	// alone already stops the key from being trusted, and as no controller completes the
	// destroy for this type, it would otherwise leave no audit trace. The hooks are
	// deliberately not marked to log the internal actor, so the periodic key rotation stays
	// out of the audit log.
	audit.ShouldLogUpdate(a, oidc.JWTPublicKeyType, emptyUpdateFunc)
	audit.ShouldLogDestroy(a, oidc.JWTPublicKeyType, emptyDestroyFunc)
