...
func TestInfraProviderValidation(t *testing.T) {
	...
	machine.Metadata().Annotations().Set(omnires.LabelInfraProviderID, provider.Metadata().ID())
	...
	machine2.Metadata().Annotations().Set(omnires.LabelInfraProviderID, "different-provider")
	require.NoError(t, st.Create(ctx, machine2))

	machine3 := omnires.NewMachine(resources.DefaultNamespace, "machine3")
	machine3.Metadata().Annotations().Set(omnires.LabelInfraProviderID, "non-static-provider")
