...
func TestGetMachineJoinConfigMachineLabels(t *testing.T) {
	...
	// NewJoinOptions also parses the join token, so a bad token must keep its own cause rather
	// than being reported as a label problem
	t.Run("a malformed join token is not reported as a label error", func(t *testing.T) {
		_, err := server.GetMachineJoinConfig(ctx, &management.GetMachineJoinConfigRequest{
			JoinToken:     "v2:this-is-not-base64!!",
			MachineLabels: map[string]string{"env": "prod"},
		})

		require.Error(t, err)
		require.NotContains(t, err.Error(), "invalid machine labels")
	})
