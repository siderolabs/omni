...
// AssertExtensionsAreEqual asserts that the given extensions are equal to the extensions installed on all the machines of the given cluster.
func AssertExtensionsAreEqual(ctx context.Context, cli *client.Client, cluster string, extensions []string) TestFunc {
...
func checkExtensions(ctx context.Context, cli *client.Client, machineID string, extensions []string) error {
	...
	if !slices.Equal(collectedExtensions, extensions) {
		return fmt.Errorf("extensions do not match on machine %q: expected %q, got %q", machineID, extensions, collectedExtensions)
	}
...
func fetchExtensions(ctx context.Context, cli *client.Client, machineID resource.ID) ([]string, error) {
	...
	extensions := make([]string, 0, list.Len())
