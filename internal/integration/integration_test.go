...
func runOmni(t *testing.T) (string, error) {
	...
	params.Account.ID = uuid.NewString()
	...
	// Omni won't create the users as we created the service account already.
	// So doing it explicitly here.
	for _, email := range config.Auth.Auth0.InitialUsers {
		if err := user.Ensure(ctx, state.Default(), email, role.Admin); err != nil {
			assert.NoError(t, user.Ensure(ctx, state.Default(), email, role.Admin))
		}
	}
