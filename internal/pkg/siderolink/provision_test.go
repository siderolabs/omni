...
func TestGrpcTunnelPendingHandoffKeepsLinkPeer(t *testing.T) {
	...
	// subnet, and the same virtual address-port.
	...
	assert.Equal(t, pendingVA, linkVA, "the link must get the same virtual address-port")
...
func TestProvisionWithLabels(t *testing.T) {
	...
	// the UI destroys the MachineLabels resource outright when the last label is removed, so its
	// absence cannot mean "never seeded" - otherwise every reboot would hand back the labels the
	// user deleted
	t.Run("deleted labels do not come back on the next boot", func(t *testing.T) {
		t.Parallel()
		...
		token, tokenErr := jointoken.NewWithLabels(labelsTestDefaultToken, map[string]string{"env": "prod"}, nil)
		require.NoError(t, tokenErr)

		req := labelsTestRequest(t, "relabeled-machine", token)
		...
		// the user removes the last label, which deletes the whole resource
		require.NoError(t, st.Destroy(ctx, omni.NewMachineLabels(req.NodeUuid).Metadata()))

		// the machine reboots and provisions again with the very same token
		_, err = provisionHandler.Provision(ctx, req)
		require.NoError(t, err)

		_, err = st.Get(ctx, omni.NewMachineLabels(req.NodeUuid).Metadata())

		require.Error(t, err, "the deleted labels were seeded again")
		require.True(t, state.IsNotFoundError(err), "unexpected error: %v", err)
	})
...
func TestProvisionTokenFingerprint(t *testing.T) {
	...
	// a version 1 provider token is signed with the default join token, so it resolves through the
	// default branch. Recording that token ID would make MachineJoinConfigController re-encode the
	// config as version 2 - which the provider branch then validates against the provider secret
	// and rejects - so the blob the machine presented has to be kept verbatim.
	t.Run("v1 provider tokens keep the presented token", func(t *testing.T) {
		t.Parallel()
		...
		token, tokenErr := jointoken.NewWithExtraData(labelsTestDefaultToken, jointoken.Version1, map[string]string{
			omni.LabelInfraProviderID: providerID,
		})
		require.NoError(t, tokenErr)
		...
		req := labelsTestRequest(t, "legacy-provider-machine", token)
		...
		rtestutils.AssertResources(
			ctx, t, st, []string{req.NodeUuid},
			func(r *siderolinkres.JoinTokenUsage, assertion *assert.Assertions) {
				assertion.Equal(encoded, r.TypedSpec().Value.TokenId,
					"a provider token must be recorded verbatim, not as the default token ID")
			},
		)

		// the provider label still reaches the link, so the machine keeps its provider identity
		rtestutils.AssertResources(
			ctx, t, st, []string{req.NodeUuid},
			func(r *siderolinkres.Link, assertion *assert.Assertions) {
				got, ok := r.Metadata().Labels().Get(omni.LabelInfraProviderID)
				assertion.True(ok)
				assertion.Equal(providerID, got)
			},
		)
	})
	...
	t.Run("a fingerprint collision resolves by the signature", func(t *testing.T) {
		...
		// the decoy is revoked: the signature is checked before the state, so it is skipped rather
		// than shadowing the active token it collides with, whatever the iteration order is
		registerJoinToken(ctx, t, st, decoyToken, "decoy", specs.JoinTokenStatusSpec_REVOKED)
