...
func (manager *Manager) ensureDefaultJoinToken(ctx context.Context, token string) error {
	joinTokens, err := safe.ReaderListAll[*auth.JoinToken](ctx, manager.state)
	if err != nil {
		return err
	}
	...
	defaultJoinToken := auth.NewDefaultJoinToken()
	...
	if err = manager.state.Create(ctx, defaultJoinToken); err != nil {
		...
		if _, err = safe.StateUpdateWithConflicts(ctx, manager.state, defaultJoinToken.Metadata(), func(res *auth.DefaultJoinToken) error {
	...
	joinToken := auth.NewJoinToken(resources.DefaultNamespace, token)
