...
func establishLink[T res](ctx context.Context, logger *zap.Logger, st state.State, provisionContext *provisionContext,
	...
	tokenUsage := siderolink.NewJoinTokenUsage(link.Metadata().ID())
	...
	if err = st.Create(ctx, tokenUsage); err != nil {
		...
		_, err = safe.StateUpdateWithConflicts(ctx, st, tokenUsage.Metadata(), func(res *siderolink.JoinTokenUsage) error {
...
func (h *ProvisionHandler) validateV1Token(ctx context.Context, linkToken jointoken.JoinToken) (*jointoken.JoinToken, error) {
	var joinToken string

	if providerID, ok := linkToken.ExtraData[omni.LabelInfraProviderID]; ok {
...
func (h *ProvisionHandler) getJoinToken(ctx context.Context, tokenString string) (*jointoken.JoinToken, error) {
	...
	if linkToken.Version == jointoken.Version1 {
		return h.validateV1Token(ctx, linkToken)
	}
