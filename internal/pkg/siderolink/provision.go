	...
	for {
		...
		case <-timer.C:
			...
			for machine := range pendingMachines.All() {
				if time.Since(machine.Metadata().Updated()) > time.Second*30 {
					...
					h.logger.Info("cleaned up the pending machine link after grace period", zap.String("id", machine.Metadata().ID()))
				}
			}
		}
	}
...
func (h *ProvisionHandler) removePendingMachine(ctx context.Context, pendingMachine *siderolink.PendingMachine) error {
	...
	_, err := h.state.Teardown(ctx, pendingMachine.Metadata())
	if err != nil {
		if state.IsNotFoundError(err) {
			return nil
		}
	...
	_, err = h.state.WatchFor(ctx, pendingMachine.Metadata(), state.WithFinalizerEmpty())
	if err != nil {
		if state.IsNotFoundError(err) {
			return nil
		}
