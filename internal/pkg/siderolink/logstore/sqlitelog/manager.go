...
const schemaTmpl = `
    ...
    CREATE INDEX IF NOT EXISTS idx_{{.TableName}}_{{.CreatedAtColumn}} 
    ON {{.TableName}}({{.CreatedAtColumn}});
`
...
func NewManager(ctx context.Context, db *sql.DB, config config.LogsMachineSQLite, logger *zap.Logger) (*StoreManager, error) {
