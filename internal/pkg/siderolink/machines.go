...
func (m *MachineCache) init(ctx context.Context) error {
	...
	if m.logStorageConfig.SQLite.Enabled {
		...
		if logStoreManager, err = sqlitelog.NewManager(ctx, m.secondaryStorageDB, m.logStorageConfig.SQLite, m.logger); err != nil {
		...
		logStoreManager = circularlog.NewStoreManager(m.logStorageConfig, m.compressor, m.logger)
	}
