...
const (
	defaultEmail = "integration" + access.ServiceAccountNameSuffix
)
...
func CreateServiceAccount(ctx context.Context, name string, state state.State) (string, error) {
	...
	_, err = omnisa.Create(ctx, state, name, string(role.Admin), false, []byte(armoredPublicKey))
	if err != nil {
		return "", err
	}
