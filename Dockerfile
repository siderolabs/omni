...
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni/go/pkg GCFLAGS=""; [ "$WITH_DEBUG" = "1" ] && GCFLAGS="all=-N -l"; go build -gcflags="$GCFLAGS" ${GO_BUILDFLAGS} -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=acompat -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /acompat-linux-amd64
...
ARG TAG
ARG WITH_DEBUG=false
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni/go/pkg GCFLAGS=""; [ "$WITH_DEBUG" = "1" ] && GCFLAGS="all=-N -l"; go build -gcflags="$GCFLAGS" ${GO_BUILDFLAGS} -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=make-cookies -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /make-cookies-linux-amd64
...
ARG TAG
ARG WITH_DEBUG=false
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni/go/pkg GCFLAGS=""; [ "$WITH_DEBUG" = "1" ] && GCFLAGS="all=-N -l"; GOARCH=amd64 GOOS=darwin go build -gcflags="$GCFLAGS" ${GO_BUILDFLAGS} -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=omni -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /omni-darwin-amd64
...
ARG TAG
ARG WITH_DEBUG=false
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni/go/pkg GCFLAGS=""; [ "$WITH_DEBUG" = "1" ] && GCFLAGS="all=-N -l"; GOARCH=arm64 GOOS=darwin go build -gcflags="$GCFLAGS" ${GO_BUILDFLAGS} -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=omni -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /omni-darwin-arm64
...
ARG TAG
ARG WITH_DEBUG=false
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni/go/pkg GCFLAGS=""; [ "$WITH_DEBUG" = "1" ] && GCFLAGS="all=-N -l"; GOARCH=amd64 GOOS=linux go build -gcflags="$GCFLAGS" ${GO_BUILDFLAGS} -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=omni -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /omni-linux-amd64
...
ARG TAG
ARG WITH_DEBUG=false
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni/go/pkg GCFLAGS=""; [ "$WITH_DEBUG" = "1" ] && GCFLAGS="all=-N -l"; GOARCH=arm64 GOOS=linux go build -gcflags="$GCFLAGS" ${GO_BUILDFLAGS} -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=omni -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /omni-linux-arm64
...
ARG TAG
ARG WITH_DEBUG=false
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni/go/pkg GCFLAGS=""; [ "$WITH_DEBUG" = "1" ] && GCFLAGS="all=-N -l"; GOARCH=amd64 GOOS=darwin go build -gcflags="$GCFLAGS" ${GO_BUILDFLAGS} -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=omnictl -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /omnictl-darwin-amd64
...
ARG TAG
ARG WITH_DEBUG=false
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni/go/pkg GCFLAGS=""; [ "$WITH_DEBUG" = "1" ] && GCFLAGS="all=-N -l"; GOARCH=arm64 GOOS=darwin go build -gcflags="$GCFLAGS" ${GO_BUILDFLAGS} -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=omnictl -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /omnictl-darwin-arm64
...
ARG TAG
ARG WITH_DEBUG=false
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni/go/pkg GCFLAGS=""; [ "$WITH_DEBUG" = "1" ] && GCFLAGS="all=-N -l"; GOARCH=amd64 GOOS=linux go build -gcflags="$GCFLAGS" ${GO_BUILDFLAGS} -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=omnictl -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /omnictl-linux-amd64
...
ARG TAG
ARG WITH_DEBUG=false
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni/go/pkg GCFLAGS=""; [ "$WITH_DEBUG" = "1" ] && GCFLAGS="all=-N -l"; GOARCH=arm64 GOOS=linux go build -gcflags="$GCFLAGS" ${GO_BUILDFLAGS} -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=omnictl -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /omnictl-linux-arm64
...
ARG TAG
ARG WITH_DEBUG=false
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni/go/pkg GCFLAGS=""; [ "$WITH_DEBUG" = "1" ] && GCFLAGS="all=-N -l"; GOARCH=amd64 GOOS=windows go build -gcflags="$GCFLAGS" ${GO_BUILDFLAGS} -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=omnictl -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /omnictl-windows-amd64.exe
