...
} satisfies Record<HardwareType, string[]>
</script>
...
definePage({ name: 'InstallationMediaCreate' })
...
const currentStepName = computed(() => route.name)
...
const currentStep = computed(() => {
  if (!currentStepName.value || !currentFlowSteps.value) {
    return 0
  }

  return (currentFlowSteps.value as string[]).indexOf(currentStepName.value as string) + 1
})
