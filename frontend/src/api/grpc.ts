...
import {
  ResourceService as WrappedResourceService,
  GetRequest,
  ListRequest,
  CreateRequest,
  UpdateRequest,
...
import { Metadata as TalosMetadata } from "@/api/common/common.pb";
import { ref, Ref } from 'vue';
import { Code } from '@/api/google/rpc/code.pb';
import { fetchOption, NotifyStreamEntityArrival, setCommonFetchOptions } from '@/api/fetch.pb';
import { withAbortController, withPathPrefix } from '@/api/options';

export const initState = () => {
  setCommonFetchOptions(
    withPathPrefix("/api")
  );
}
...
export const subscribe = <R, T>(method: StreamingRequest<R, T>, params: T, handler: NotifyStreamEntityArrival<R>, options?: fetchOption[], onStart?: () => void, onError?: (e: Error) => void, onFinish?: () => void) => {
...
export class Stream<R, T> {
  ...
  constructor(method: StreamingRequest<R, T>, params: T, handler: NotifyStreamEntityArrival<R>, options?: fetchOption[], onStart?: () => void, onError?: (e: Error) => void, onFinish?: () => void) {
    const opts = options || [];
    let currentDelay = 0;
    let retryCount = 0;
...
export class ResourceService {
  ...
  static async List<T extends Resource>(request: ListRequest, ...options: fetchOption[]): Promise<T[]> {
  ...
  static async Teardown(request: DeleteRequest, ...options: fetchOption[]): Promise<DeleteResponse> {
    return checkError(await WrappedResourceService.Teardown(request, ...options));
  }

  static async Watch(request: WatchRequest, callback: NotifyStreamEntityArrival<WatchResponse>, options?: fetchOption[], onStart?: () => void, onError?: (e: Error) => void): Promise<Stream<WatchRequest, WatchResponse>> {
