...
export const addMachineLabels = async (machineID: string, ...labels: string[]) => {
  ...
  let exists = true
  try {
    resource = await ResourceService.Get(resource.metadata, withRuntime(Runtime.Omni))
  } catch (e) {
...
export const removeMachineLabels = async (machineID: string, ...keys: string[]) => {
  ...
  try {
    ...
    const machineStatus = await ResourceService.Get(
      { ...metadata, type: MachineStatusType },
      withRuntime(Runtime.Omni),
    )
...
export const updateMachineLock = async (id: string, locked: boolean) => {
  ...
  if (!machine.metadata.annotations) machine.metadata.annotations = {}
...
export const updateInfraMachineConfig = async (
  ...
  try {
    const resource: Resource<InfraMachineConfigSpec> = await ResourceService.Get(
      metadata,
      withRuntime(Runtime.Omni),
    )
    ...
    if (e.code === Code.NOT_FOUND) {
      ...
      await ResourceService.Create<Resource<InfraMachineConfigSpec>>(
        resource,
        withRuntime(Runtime.Omni),
      )
    }
