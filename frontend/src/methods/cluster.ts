...
const updateResource = async <T extends Resource>(
  ...
  await ResourceService.Update<T>(
    resource,
...
export const destroyNodes = async (
  ...
  try {
    ...
    for (const item of resp) {
      ...
      if (
        item.metadata.labels[LabelMachineSet] ===
...
export const nextAvailableClusterName = async (
  prefix: string
): Promise<string> => {
...
export const upgradeKubernetes = async (
  clusterName: string,
  version: string
) => {
...
export const revertKubernetesUpgrade = async (clusterName: string) => {
  const upgradeStatus: Resource<
    TalosUpgradeStatusSpec | KubernetesUpgradeStatusSpec
  ...
  await upgradeKubernetes(
    clusterName,
    upgradeStatus.spec.last_upgrade_version!
  );
};
...
export const addClusterLabels = async (
  clusterID: string,
  ...labels: string[]
) => {
...
export const removeClusterLabels = async (
  clusterID: string,
  ...keys: string[]
) => {
  const resource: Resource<ClusterSpec> = await ResourceService.Get(
...
export const setClusterWorkloadProxy = async (
  clusterID: string,
  enabled: boolean
) => {
  const resource: Resource<ClusterSpec> = await ResourceService.Get(
...
export const setUseEmbeddedDiscoveryService = async (
  clusterID: string,
  enabled: boolean
) => {
  const resource: Resource<ClusterSpec> = await ResourceService.Get(
...
export const setClusterEtcdBackupsConfig = async (
  clusterID: string,
  spec: ClusterSpec
) => {
  const resource: Resource<ClusterSpec> = await ResourceService.Get(
...
export const triggerEtcdBackup = async (clusterID: string) => {
  ...
  try {
    manualBackup = await ResourceService.Get(
      metadata,
      withRuntime(Runtime.Omni)
    );
  ...
  return await ResourceService.Update(
    manualBackup,
    undefined,
    withRuntime(Runtime.Omni)
  );
