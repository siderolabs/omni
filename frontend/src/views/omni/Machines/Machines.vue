...
const panelMachine = ref<Resource<MachineStatusLinkSpec>>()
  ...
  >
    ...
    <template #default="{ items, searchQuery }">
      <div class="flex h-full overflow-hidden">
        <div class="z-0 min-w-0 grow overflow-auto">
          <MachineItem
            v-for="item in items"
            :key="item.metadata.id"
            :machine="item"
            :search-query="searchQuery"
            :panel-open="open && item.metadata.id === panelMachine?.metadata.id"
            :selected="selectedMachines.has(item.metadata.id ?? '')"
            @update:selected="(v) => updateSelected(item, v)"
            @open-panel="((panelMachine = item), (open = true))"
        ...
        <MachineDetailsPanel
          ...
          :class="open ? 'max-lg:w-full lg:w-sm' : 'pointer-events-none opacity-0 lg:w-0'"
          @close="open = false"
        />
      </div>
    </template>
