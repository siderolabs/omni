    ...
    <div class="flex flex-col w-full h-full gap-4">
      ...
      <template v-if="!key">
        <div class="flex flex-col gap-2">
          <t-input title="Name" class="flex-1 h-full" v-model="name" :onClear="() => label = ''"/>
          <t-input title="ID" class="flex-1 h-full" v-model="id" :onClear="() => id = ''"/>
...
import { JoinTokenLen } from "@/api/resources";
...
const id = ref(makeid(JoinTokenLen));
...
const handleCreate = async () => {
  if (id.value === "") {
    showError("Failed to Create Join Token", "ID is not defined", notification);
  ...
  try {
    await createJoinToken(id.value, name.value, lifetime.value === Lifetime.Limited ? expiration.value : undefined);
  } catch (e) {
