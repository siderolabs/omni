    ...
    msw: {
      handlers: [
        ...
        http.post<never, CreateSchematicRequest>(
          ...
          async ({ request }) => {
            const { secure_boot, talos_version } = await request.clone().json()
            ...
            return HttpResponse.json<CreateSchematicResponse>({
              ...
              schematic_yml: dump({
                customization: {
                  extraKernelArgs: [
                    'siderolink.api=grpc://192.168.1.175:8090?grpc_tunnel=true&jointoken=w7uVuW3zbVKIYQuzEcyetAHeYMeo5q2L9RvkAVfCfSCD',
