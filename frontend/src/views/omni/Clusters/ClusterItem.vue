...
-->
<template>
  <list-item-box :itemID="item.metadata.id!" :id="`${item.metadata.id}-cluster-box`" listID="cluster" ref="box">
    <template #default>
      <div class="clusters-grid flex-1">
        <div class="list-item-link">
          <router-link :to="{ name: 'ClusterOverview', params: { cluster: item?.metadata.id } }">
            <word-highlighter :query="(searchQuery ?? '')" :textToHighlight="item.metadata.id" split-by-space
              highlightClass="bg-naturals-N14" />
          </router-link>
        </div>
        <div class="ml-3" id="machine-count">{{ item?.spec?.machines?.healthy ?? 0 }}/{{ item?.spec?.machines?.total ??
          0 }}</div>
        <div />
        <item-labels class="ml-5" v-if="item" :resource="item" :add-label-func="addClusterLabels"
          :remove-label-func="removeClusterLabels" @filter-label="(e) => $emit('filterLabels', e)" />
      </div>
      ...
      <t-actions-box style="height: 24px" @click.stop>
        <t-actions-box-item icon="nodes"
          @click="() => $router.push({ name: 'ClusterScale', params: { cluster: item?.metadata?.id } })"
          v-if="canAddClusterMachines">Cluster Scaling
        </t-actions-box-item>
        <t-actions-box-item icon="settings"
          @click="() => $router.push({ name: 'ClusterConfigPatches', params: { cluster: item?.metadata?.id } })">Config
          Patches
        </t-actions-box-item>
        <t-actions-box-item icon="dashboard"
          @click="() => $router.push({ name: 'ClusterOverview', params: { cluster: item?.metadata?.id } })">Open
          Dashboard
        </t-actions-box-item>
        <t-actions-box-item icon="kube-config" @click="() => downloadKubeconfig(item?.metadata?.id as string)"
          v-if="canDownloadKubeconfig">Download <code>kubeconfig</code></t-actions-box-item>
        <t-actions-box-item icon="talos-config" @click="() => downloadTalosconfig(item?.metadata?.id)"
          v-if="canDownloadTalosconfig">Download <code>talosconfig</code></t-actions-box-item>
        <t-actions-box-item icon="delete" danger @click="openClusterDestroy" v-if="canRemoveClusterMachines">Destroy
          Cluster</t-actions-box-item>
      </t-actions-box>
    </template>
    <template #details>
