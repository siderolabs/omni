...
<template>
  <div class="flex flex-col gap-2">
    <div class="flex justify-end">
      <t-button @click="openInfraProviderSetup" icon-position="left" type="highlighted" :disabled="!canManageUsers">New Service Provider Setup</t-button>
    </div>
    <t-list
      :opts="watchOpts"
      search
    ...
    >
      <template #default="{ items }">
        <div class="flex flex-col gap-2">
          <list-item-box v-for="item in items" listID="providers" :itemID="item.metadata.id!" :key="itemID(item)"
            ...
            <template #default>
              <div class="flex w-full">
                <div class="providers-grid">
                  <div class="flex gap-3 items-center text-md text-naturals-N13">
                    ...
                    <div class="flex flex-col gap-0.5">
                      ...
                      <div class="text-xs text-naturals-N10 font-bold">ID: {{ item.metadata.id! }}</div>
                    </div>
                  </div>
                ...
                <icon-button icon="delete" danger @click="() => openInfraProviderDelete(item.metadata.id!)"/>
              </div>
            </template>
          </list-item-box>
        </div>
...
import {  selectors, Label } from "@/methods/labels";
import { DefaultNamespace, EphemeralNamespace, IdentityType, InfraProviderCombinedStatusType, InfraProviderServiceAccountDomain, RoleInfraProvider } from "@/api/resources";
...
import { InfraProviderCombinedStatusSpec } from "@/api/omni/specs/omni.pb";
