...
<template>
  <div class="overview">
    ...
    <ul class="overview-data-list">
      <li class="overview-data-item">
        ...
        <div class="overview-data-row">
          ...
          <div class="overview-data-roles">
            <tag class="overview-data-role" v-for="role in roles" :key="role">
              {{ role }}
            </tag>
          </div>
        </div>
      ...
      <li class="overview-data-item">
        ...
        <div class="overview-data-row" v-if="talosMachineStatus">
          ...
          />
          <div v-else class="text-xs flex gap-1 items-center text-green-G1">
            <t-icon icon="check-in-circle-classic" class="h-4" />None
          </div>
        </div>
    ...
    <template v-if="machineStatus">
      <ul class="overview-data-list" v-if="machineStatus.spec.message_status?.diagnostics">
        <li class="bg-naturals-N2 w-full rounded flex flex-col">
          ...
          <node-diagnostic-warnings
            :diagnostics="machineStatus?.spec?.message_status?.diagnostics"
          />
        </li>
      </ul>
    ...
    <div class="overview-services">
      ...
      <t-group-animation>
        <t-list-item v-for="service in services" :key="service.name">
          <template #default>
            <div class="grid grid-cols-4 p-1">
              <router-link
                class="col-span-2 list-item-link"
                :to="{
                  name: 'NodeLogs',
                  query: {
