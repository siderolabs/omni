...
func defineServiceFlags() {
	...
	// Services.Embedded discovery service
	rootCmd.Flags().BoolVar(
...
func defineAuthFlags() {
	...
	rootCmd.Flags().StringVar(&cmdConfig.Auth.SAML.URL, "auth-saml-url", cmdConfig.Auth.SAML.URL, "SAML identity provider metadata URL (mutually exclusive with --auth-saml-metadata")
