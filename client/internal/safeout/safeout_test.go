...
// payloads that can be put into any string omnictl renders.
var payloads = map[string]struct {
	...
	"CSI screen repaint": {
		in:       "\x1b[2J\x1b[HF3-SCREEN-REPAINTED-BY-SERVER",
		expected: `\x1b[2J\x1b[HF3-SCREEN-REPAINTED-BY-SERVER`,
	},
...
// TestWriterSplitRune covers the case a stream filter gets wrong: log output
...
func TestCellEscapesTabAndNewline(t *testing.T) {
	...
	// value containing either rewrites the shape of the table around it.
...
// TestNoControlRunesSurvive asserts the property the package is about directly,
