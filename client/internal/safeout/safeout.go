...
//
// Most of what omnictl prints comes from the Omni API, and much of that originates on
// the machines Omni manages: machine logs, resource fields, hostnames, error messages.
// Any of it can carry terminal control sequences, and a terminal acts on them: OSC 52
// writes the operator's clipboard, a bare CR overwrites the line prefix omnictl printed
// itself, CSI sequences repaint the screen, and bidirectional overrides reorder what the
// operator reads. The only place to separate the data from the terminal is here, on the
// client, at the point where omnictl renders.
//
// The filter is the identity transformation on printable UTF-8: well-behaved output passes
// through byte for byte, so piping omnictl into jq, grep or a file keeps working and only
// malicious or malformed content is rewritten. That property is what makes it safe to
// filter unconditionally rather than only when stdout is a terminal - logs are routinely
// redirected to a file or captured by CI and rendered on a terminal much later, which is
// precisely when a terminal-detection check would have already waved the payload through.
//
// The filter deliberately does not escape a literal backslash, so an escape it emits is
// indistinguishable from that same text appearing literally in the source data. Preserving
// the identity property is worth more than that ambiguity: neither form can drive a terminal.
//
// Escaping is not the right answer for every kind of output, and this package is not meant
// for all of it:
//
//   - Structured output (-o json, -o yaml) is already safe, because the encoders escape
//     control characters themselves. Do not filter it again.
//   - Byte streams that are not text at all (installation media, support bundles) must reach
//     their destination unmodified. Those are written to files, never to the terminal.
//   - Escape sequences omnictl emits itself - colors, spinners - must be written outside this
//     filter, otherwise omnictl escapes its own styling. Filter the untrusted field with
//     [String] or [Cell] before interpolating it into a line that is styled later.
package safeout
...
// RawOutputEnvVar disables filtering of the streams when set to a true value, for an
// operator who needs the original bytes (a service which legitimately colorizes its
// own log output, say).
//
// It does not disable [String] and [Cell]: a field interpolated into a line
// omnictl formats and colorizes itself is escaped either way, as leaving that
// one field raw is the injection this package exists to prevent.
const RawOutputEnvVar = "OMNICTL_RAW_OUTPUT"
...
// NewWriter returns a Writer filtering everything written to w, unless raw output is requested via [RawOutputEnvVar].
func NewWriter(w io.Writer) *Writer {
	return &Writer{w: w, raw: rawOutput()}
}
...
var (
	stdout = sync.OnceValue(func() *Writer { return &Writer{w: os.Stdout, raw: rawOutput()} }) //nolint:forbidigo // the raw stream underneath the filter
	stderr = sync.OnceValue(func() *Writer { return &Writer{w: os.Stderr, raw: rawOutput()} }) //nolint:forbidigo // the raw stream underneath the filter
)

// Stdout returns the filtered standard output stream.
//
// Anything rendering text received from the API for a human writes here instead of
// to os.Stdout, including as the writer underneath a tabwriter.
...
// because it is omnictl's own literal, so the tabs which separate the columns
// keep their meaning. The values interpolated into it come from the API, and a
...
func Print(a ...any) { fmt.Fprint(Stdout(), a...) } //nolint:errcheck

// Printf is fmt.Printf against the filtered standard output stream.
func Printf(format string, a ...any) { fmt.Fprintf(Stdout(), format, a...) } //nolint:errcheck

// Println is fmt.Println against the filtered standard output stream.
func Println(a ...any) { fmt.Fprintln(Stdout(), a...) } //nolint:errcheck
...
// that omnictl's own escape sequences survive while the received ones do not.
...
// newline ends the row, so a value containing either one silently rewrites the
// shape of the table around it.
