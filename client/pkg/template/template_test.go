...
func TestLoad(t *testing.T) {
	for _, tt := range []struct { //nolint:govet
		...
		},
		{
			...
			expectedError: "error decoding document at line 1:1: yaml: construct errors: line 7: field containerd not found in type models.Cluster",
		},
		{
...
func TestDelete(t *testing.T) {
	...
	// deleting the template releases the machine-scoped install disk config instead of destroying
	// it: the selection is a machine property that survives cluster membership
	assert.Equal(t, []string{
		"MachineInstallDiskConfigs.omni.sidero.dev(default/430d882a-51a8-48b3-ae00-90c5b0b5b0b0)",
...
func TestSync_InstallDisk_Lifecycle(t *testing.T) {
	...
	// removing the install stanza while the machine stays in the template destroys the config:
	// that is the only way the template author can return the machine to automatic selection
