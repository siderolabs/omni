...
func getOperationForNoMoreExpectedResource(actualResource resource.Resource, clusterName string, expectedMachineSetNodeIDs map[resource.ID]struct{}) (
	...
	case omni.MachineInstallDiskConfigType:
		...
		// The install stanza was removed while the machine stays in the template: the author asked
		// to return to automatic disk selection, which only deleting the resource can express.
		...
		// The machine left the template or the template is being deleted: release ownership and
		// keep the resource, the selection is a machine property that survives cluster membership.
