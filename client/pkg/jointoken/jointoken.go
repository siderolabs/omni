...
// ErrInvalidLabels is returned when the labels a join token should carry are not acceptable,
// either on their own or because they make the encoded token exceed MaxEncodedTokenLen.
var ErrInvalidLabels = errors.New("invalid machine labels")
...
func newSigned(t JoinToken) (JoinToken, error) {
	...
	if len(encoded) > MaxEncodedTokenLen {
		sizeErr := fmt.Errorf("the encoded join token is too long: %d bytes (max %d)", len(encoded), MaxEncodedTokenLen)

		if len(t.Labels) > 0 {
			return JoinToken{}, fmt.Errorf("%w: %w, reduce the number or the size of the labels", ErrInvalidLabels, sizeErr)
		}

		return JoinToken{}, sizeErr
	}
...
// ValidateLabels reports whether the labels may be carried by a join token.
//
// Keys carrying the omni.sidero.dev/ prefix are rejected: those are Omni's to set, and the labels
// a token carries become the user's own labels on the machine.
func ValidateLabels(labels map[string]string) error {
	for key, value := range labels {
		if strings.HasPrefix(key, omni.SystemLabelPrefix) {
			return fmt.Errorf("%w: label %q is not allowed: the %q prefix is reserved for the system labels",
				ErrInvalidLabels, key, omni.SystemLabelPrefix)
		}

		if err := omni.ValidateLabel(key, value); err != nil {
			return fmt.Errorf("%w: %w", ErrInvalidLabels, err)
		}
	}
...
func Parse(value string) (JoinToken, error) {
	...
	if res.Version != VersionPlain {
		// the encoded form is bounded on the way out, so bound it on the way in too: the signature
		// only proves who wrote the token, and nothing downstream limits how much a hand crafted
		// one may carry
		if len(value) > MaxEncodedTokenLen {
			return res, fmt.Errorf("the join token is too long: %d bytes (max %d)", len(value), MaxEncodedTokenLen)
		}
		...
		// the version 1 and 2 signature covers the extra data alone, so anyone holding such a
		// token could append these fields to it and keep the signature valid. They are only
		// meaningful from version 3 on, where the signature covers them.
		if res.Version != Version3 && (res.Labels != nil || res.TokenFingerprint != "") {
			return res, fmt.Errorf("join token version %s must not carry labels or a token fingerprint", res.Version)
		}

		// the constructors validate on the way out, but a token holder can craft the payload by
		// hand, and the provision handler writes the labels to a state which does not run the
		// resource metadata validations
