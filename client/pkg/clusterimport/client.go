...
func extractVersion(imageRef string) (string, error) {
	...
	v, err := semver.ParseTolerant(strings.TrimLeft(imageRef[idx+1:], "v"))
	if err != nil {
		return "", fmt.Errorf("failed to parse image reference %q: %w", imageRef, err)
	}

	if version == nil || v.LT(*version) { //nolint:govet
		version = &v
	}

	if version == nil {
		return "", errors.New("failed to detect version from image reference")
	}
...
func BuildTalosClient(ctx context.Context, cluster, config, context, sideroV1KeysDir string, endpoints []string) (TalosClient, error) {
	...
	opts = append(opts, talosclient.WithCluster(cluster))
