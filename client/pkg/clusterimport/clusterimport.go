...
func BuildContext(ctx context.Context, input Input, omniState state.State, imageFactoryClient ImageFactoryClient, talosClient TalosClient) (*Context, error) {
	...
	firstNodeInfo := nodeInfoMap[firstNode]

	firstNodeKubeletVersion, err := extractVersion(firstNodeInfo.kubeletConfig.TypedSpec().Image)
	if err != nil {
		return nil, fmt.Errorf("failed to get kubelet version from node %q: %w", firstNode, err)
	}

	err = input.initVersions(firstNodeInfo.hostname, firstNodeInfo.talosVersion.Tag, firstNodeKubeletVersion)
	if err != nil {
		return nil, err
	}

	if semver.MustParse(constants.MinTalosVersion).GTE(semver.MustParse(input.TalosVersion)) {
		return nil, fmt.Errorf("minimum required version of talos is %s", constants.MinTalosVersion)
	}

	talosVersionMatrix, err := safe.ReaderGetByID[*omni.TalosVersion](ctx, omniState, omni.NewTalosVersion(resources.DefaultNamespace, input.TalosVersion).Metadata().ID())
	if err != nil {
		return nil, fmt.Errorf("failed to read talos version: %w", err)
	}

	if talosVersionMatrix == nil {
		return nil, fmt.Errorf("talos version %q not found in omni", input.TalosVersion)
	}

	if !slices.Contains(talosVersionMatrix.TypedSpec().Value.CompatibleKubernetesVersions, input.KubernetesVersion) {
		return nil, fmt.Errorf("talos version %q is not compatible with kubernetes version %q", input.TalosVersion, input.KubernetesVersion)
	}

	return newContext(input, talosCli, imageFactoryClient, nodeInfoMap, joinOpts, omniState)
}
...
func newContext(
	...
	ics := *omni.NewImportedClusterSecrets(resources.DefaultNamespace, input.ClusterID)
	...
	cluster := *omni.NewCluster(resources.DefaultNamespace, input.ClusterID)
	cluster.Metadata().Annotations().Set(omni.ClusterLocked, "")
	cluster.Metadata().Annotations().Set(omni.ClusterImportIsInProgress, "")
	cluster.TypedSpec().Value.TalosVersion, _ = strings.CutPrefix(input.TalosVersion, "v")
	cluster.TypedSpec().Value.KubernetesVersion, _ = strings.CutPrefix(input.KubernetesVersion, "v")

	controlPlaneMachineSet := *omni.NewMachineSet(resources.DefaultNamespace, input.ClusterID+"-control-planes")
	controlPlaneMachineSet.Metadata().Labels().Set(omni.LabelCluster, input.ClusterID)
	controlPlaneMachineSet.Metadata().Labels().Set(omni.LabelControlPlaneRole, "")

	workerMachineSet := *omni.NewMachineSet(resources.DefaultNamespace, input.ClusterID+"-workers")
	workerMachineSet.Metadata().Labels().Set(omni.LabelCluster, input.ClusterID)
	...
	for _, node := range input.Nodes {
		...
		if info.isControlPlane {
			...
			machineSetNode.Metadata().Labels().Set(omni.LabelCluster, input.ClusterID)
		...
		genOutput, nodeErr := machineconfig.Generate(machineconfig.GenerateInput{
			...
			InitialTalosVersion:      input.InitialTalosVersion,
			InitialKubernetesVersion: input.InitialKubernetesVersion,
		...
		for i, patch := range patches {
			...
			configPatch.Metadata().Labels().Set(omni.LabelCluster, input.ClusterID)
	...
	input.logf("importing cluster %q with %d nodes (%d control planes, %d workers)", input.ClusterID, len(input.Nodes), len(controlPlanes), len(workers))
...
func (c *Context) Run(ctx context.Context) error {
	if err := c.validate(ctx); err != nil {
		...
		c.input.logf("failed to validate cluster status %q, but continuing since 'force' is requested: %v", c.input.ClusterID, err)
	}
...
func (c *Context) validate(ctx context.Context) error {
	...
	for _, info := range c.nodeInfoMap {
		...
		if info.isControlPlane {
			version, imageErr := extractVersion(info.apiServerConfig.TypedSpec().Image)
			if imageErr != nil {
			...
			version, imageErr = extractVersion(info.controllerManagerConfig.TypedSpec().Image)
			if imageErr != nil {
			...
			version, imageErr = extractVersion(info.schedulerConfig.TypedSpec().Image)
			if imageErr != nil {
		...
		version, imageErr := extractVersion(info.kubeletConfig.TypedSpec().Image)
		if imageErr != nil {
...
func (c *Context) saveMachineConfigBackup() error {
	c.input.logf("saving machine config backups to %q", c.input.getBackupOutput())
	...
	zipFile, err := c.openArchive(c.input.getBackupOutput())
	if err != nil {
...
func (c *Context) createConfigPatches(ctx context.Context) error {
	...
	for _, cp := range c.configPatchesMap {
		if err := c.omniState.Modify(ctx, cp, func(res resource.Resource) error { return nil }); err != nil {
...
func (c *Context) createMachineSetNodes(ctx context.Context) error {
	...
	for _, msn := range c.machineSetNodesMap {
		if err := c.omniState.Modify(ctx, msn, func(res resource.Resource) error { return nil }); err != nil {
...
func (c *Context) createMachineSetWorkers(ctx context.Context) error {
	c.input.logf("creating machine set for workers")
	...
	if err := c.omniState.Modify(ctx, c.workerMachineSet, func(res resource.Resource) error { return nil }); err != nil {
...
func (c *Context) createMachineSetControlPlanes(ctx context.Context) error {
	c.input.logf("creating machine set for control planes")
	...
	if err := c.omniState.Modify(ctx, c.controlPlaneMachineSet, func(res resource.Resource) error { return nil }); err != nil {
...
func (c *Context) createCluster(ctx context.Context) error {
	...
	if err := c.omniState.Modify(ctx, c.cluster, func(res resource.Resource) error { return nil }); err != nil {
...
func (c *Context) createImportedClusterSecrets(ctx context.Context) error {
	c.input.logf("creating imported cluster secrets")
	...
	if err := c.omniState.Modify(ctx, c.importedClusterSecrets, func(res resource.Resource) error { return nil }); err != nil {
