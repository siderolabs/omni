...
// Omni >= 1.9 supports proxying through the management API; older servers require talking to the image
// factory directly using the endpoint exposed via FeaturesConfig.
func newImageFactoryClient(ctx context.Context, c *client.Client, st state.State, logger *zap.Logger) (provision.FactoryClient, error) {
	useProxied, err := supportsProxiedImageFactory(ctx, st, logger)
	if err != nil {
		return nil, err
	}
