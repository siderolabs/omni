...
func talosCA(ctx context.Context, client *client.Client) error {
	...
	if rotationStatus.TypedSpec().Value.Phase != specs.ClusterSecretsRotationStatusSpec_OK {
		if rotationStatus.TypedSpec().Value.Component != specs.ClusterSecretsRotationStatusSpec_TALOS_CA {
...
func printSecretRotationStatus(ctx context.Context, st state.State, clusterName string, rotationStatus *omni.ClusterSecretsRotationStatus) error {
	...
	for {
		...
		case event := <-watchCh:
			...
			if res.TypedSpec().Value.Phase == specs.ClusterSecretsRotationStatusSpec_OK {
