...
func abortImport(ctx context.Context, client *client.Client, clusterID resource.ID) error {
	...
	abortContext, err := clusterimport.NewAbortContext(ctx, input, omniState)
	if err != nil {
		return err
	}

	fmt.Fprintf(os.Stderr, "aborting import operation for cluster %q\n", clusterID) //nolint:errcheck

	if err = abortContext.Abort(ctx); err != nil {
...
func importCluster(ctx context.Context, client *client.Client, clusterID resource.ID) error {
	...
	if existingCluster != nil {
		return fmt.Errorf("cluster %q already exists in Omni", clusterID)
	}

	fmt.Fprintf(os.Stderr, "importing cluster %q to Omni\n", clusterID)
	...
	talosClient, err := clusterimport.BuildTalosClient(ctx, input.ClusterID, importCmdFlags.talosConfig, importCmdFlags.talosContext, access.CmdFlags.SideroV1KeysDir, importCmdFlags.endpoints)
	if err != nil {
		return err
	}
	...
	if err = importContext.Run(ctx); err != nil {
		if !errors.Is(err, clusterimport.ErrValidation) {
			return fmt.Errorf("failed to import cluster %q: %w", clusterID, err)
		}

		return fmt.Errorf("failed to validate cluster status %q, can be overridden with --%s:  %w", clusterID, flagImportForce, err)
	}
...
var importCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		return access.WithClient(func(ctx context.Context, client *client.Client) error {
			return importCluster(ctx, client, args[0])
		})
	},
}
...
func init() {
	...
	importCmd.Flags().StringSliceVarP(&input.Nodes, flagNodes, "n", []string{}, "nodes to import")
	importCmd.MarkFlagRequired(flagNodes) //nolint:errcheck
	importCmd.Flags().StringSliceVarP(&importCmdFlags.endpoints, "endpoints", "e", []string{}, "override default endpoints in talos configuration")
	importCmd.Flags().StringVarP(&importCmdFlags.talosContext, "talos-context", "", "",
		"the context to be used for talos. defaults to the selected context in the talosconfig file")
	importCmd.Flags().StringVarP(&input.TalosVersion, "talos-version", "", "",
		"talos version of the cluster, if not set, will be detected from the nodes")
	importCmd.Flags().StringVarP(&input.KubernetesVersion, "kubernetes-version", "", "",
		"kubernetes version of the cluster, if not set, will be detected from the nodes")
	importCmd.Flags().StringVarP(&input.InitialTalosVersion, "initial-talos-version", "", "",
		"initial talos version used on cluster creation, if not set current talos version will be used")
	importCmd.Flags().StringVarP(&input.InitialKubernetesVersion, "initial-kubernetes-version", "", "",
