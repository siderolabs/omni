...
const (
	...
	// BootloaderAuto/UEFI/BIOS/Dual are the user-facing bootloader strings.
	BootloaderAuto = "auto"
	BootloaderUEFI = "uefi"
	BootloaderBIOS = "bios"
	BootloaderDual = "dual"

	// GrpcTunnelAuto/Enabled/Disabled are the user-facing gRPC tunnel mode strings.
	GrpcTunnelAuto     = "auto"
	GrpcTunnelEnabled  = "enabled"
	GrpcTunnelDisabled = "disabled"
)
...
// ParseBootloader converts a bootloader string to the proto enum. Returns an error for unknown
// values so typos like "uefii" are surfaced instead of silently coerced to auto.
func ParseBootloader(bootloader string) (management.SchematicBootloader, error) {
	switch strings.ToLower(bootloader) {
	case BootloaderAuto:
		return management.SchematicBootloader_BOOT_AUTO, nil
	case BootloaderUEFI:
		return management.SchematicBootloader_BOOT_SD, nil
	case BootloaderBIOS:
		return management.SchematicBootloader_BOOT_GRUB, nil
	case BootloaderDual:
		return management.SchematicBootloader_BOOT_DUAL, nil
	default:
		return management.SchematicBootloader_BOOT_AUTO, fmt.Errorf("unknown bootloader %q, must be one of: %s, %s, %s, %s", bootloader, BootloaderAuto, BootloaderUEFI, BootloaderBIOS, BootloaderDual)
	}
}

// BootloaderToString is the inverse of ParseBootloader.
func BootloaderToString(b management.SchematicBootloader) string {
	switch b {
	case management.SchematicBootloader_BOOT_SD:
		return BootloaderUEFI
	case management.SchematicBootloader_BOOT_GRUB:
		return BootloaderBIOS
	case management.SchematicBootloader_BOOT_DUAL:
		return BootloaderDual
...
// GrpcTunnelModeToString renders a preset's gRPC tunnel mode as a user-facing string.
func GrpcTunnelModeToString(m specs.GrpcTunnelMode) string {
	switch m {
	case specs.GrpcTunnelMode_ENABLED:
		return GrpcTunnelEnabled
	case specs.GrpcTunnelMode_DISABLED:
		return GrpcTunnelDisabled
	case specs.GrpcTunnelMode_UNSET:
		fallthrough
	default:
		return GrpcTunnelAuto
	}
}
...
func ResolveOverlay(ctx context.Context, client *client.Client, overlayName, overlayOptions string) (*management.CreateSchematicRequest_Overlay, error) {
	cfg, err := safe.ReaderGetByID[*virtual.SBCConfig](ctx, client.Omni().State(), overlayName)
	if err != nil {
	...
	return &management.CreateSchematicRequest_Overlay{
		Name:    cfg.TypedSpec().Value.OverlayName,
		Image:   cfg.TypedSpec().Value.OverlayImage,
		Options: overlayOptions,
	}, nil
}

// ValidateCloudPlatform checks that the cloud platform exists and supports the given arch, secure boot, and Talos version.
func ValidateCloudPlatform(ctx context.Context, st state.State, platform string, arch specs.PlatformConfigSpec_Arch, secureBoot bool, talosVersion string) error {
	cfg, err := safe.ReaderGetByID[*virtual.CloudPlatformConfig](ctx, st, platform)
	if err != nil {
		return fmt.Errorf("failed to get cloud platform config for %q: %w", platform, err)
	}

	spec := cfg.TypedSpec().Value

	if !slices.Contains(spec.Architectures, arch) {
		return fmt.Errorf("cloud platform %q does not support architecture %q", platform, ArchToString(arch))
	}

	if secureBoot && !spec.SecureBootSupported {
		return fmt.Errorf("cloud platform %q does not support secure boot", platform)
	}

	return checkMinTalosVersion(talosVersion, spec.MinVersion, fmt.Sprintf("cloud platform %q", platform))
}

// ValidateSBC checks that the SBC overlay exists and supports the given Talos version.
func ValidateSBC(ctx context.Context, st state.State, overlay, talosVersion string) error {
	cfg, err := safe.ReaderGetByID[*virtual.SBCConfig](ctx, st, overlay)
	if err != nil {
		return fmt.Errorf("failed to get SBC config for overlay %q: %w", overlay, err)
	}

	return checkMinTalosVersion(talosVersion, cfg.TypedSpec().Value.MinVersion, fmt.Sprintf("SBC overlay %q", overlay))
}

// ValidateTalosVersion verifies the given Talos version is known to the server by checking that
// a TalosExtensions catalog exists for it.
func ValidateTalosVersion(ctx context.Context, st state.State, talosVersion string) error {
	_, err := safe.ReaderGetByID[*omni.TalosVersion](ctx, st, strings.TrimLeft(talosVersion, "v"))
	if err != nil {
		if state.IsNotFoundError(err) {
			return fmt.Errorf("unknown Talos version %q: %w", talosVersion, err)
		}

		return fmt.Errorf("failed to get Talos version %q: %w", talosVersion, err)
	}
...
func ValidateExtensions(ctx context.Context, st state.State, talosVersion string, extensions []string) error {
	...
	_, err := lookupExtensions(ctx, st, talosVersion, extensions, false)
...
func checkMinTalosVersion(actual, minVersion, source string) error {
	...
	actualV, err := semver.ParseTolerant(actual)
	if err != nil {
		return fmt.Errorf("failed to parse Talos version %q: %w", actual, err)
	}

	minV, err := semver.ParseTolerant(minVersion)
	if err != nil {
		return fmt.Errorf("failed to parse min Talos version %q from %s: %w", minVersion, source, err)
	}

	actualV.Pre = nil

	if actualV.LT(minV) {
		return fmt.Errorf("%s requires Talos version >= %s (got %s)", source, minVersion, actual)
	}
...
func BuildImageFromPreset(ctx context.Context, client *client.Client, name, arch string, opts MediaBuildOptions) (ImageInfo, error) {
	...
	case opts.Platform != "":
		...
		cloudConfig, err := safe.ReaderGetByID[*virtual.CloudPlatformConfig](ctx, client.Omni().State(), opts.Platform)
		if err != nil {
			return ImageInfo{}, fmt.Errorf("failed to get cloud platform config for %q: %w", opts.Platform, err)
		}
...
func createSchematic(ctx context.Context, client *client.Client, image ImageInfo, params Params) (*management.CreateSchematicResponse, error) {
	...
	if params.Extensions != nil {
		extensions, err = getExtensions(ctx, client.Omni().State(), params.TalosVersion, params.Extensions)
		if err != nil {
...
func getExtensions(ctx context.Context, st state.State, talosVersion string, extensions []string) ([]string, error) {
	return lookupExtensions(ctx, st, talosVersion, extensions, true)
}

func lookupExtensions(ctx context.Context, st state.State, talosVersion string, extensions []string, announce bool) ([]string, error) {
	...
	for _, extension := range extensions {
		matched := xslices.Filter(allNames, func(item string) bool {
			return strings.Contains(item, extension)
		})
