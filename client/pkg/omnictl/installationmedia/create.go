...
Examples:
    ...
    omnictl media create my-preset --arch amd64

    # Create a cloud preset for AWS
    omnictl media create aws-preset --arch amd64 --platform aws

    # Create an SBC preset for Raspberry Pi
    omnictl media create rpi-preset --arch arm64 --overlay rpi_generic

    # Create a preset with extensions and labels
    omnictl media create full-preset --arch amd64 \
...
func init() {
	...
	createCmd.Flags().StringVar(&createCmdFlags.talosVersion, flagTalosVersion, constants.DefaultTalosVersion, "Talos version to be used in the preset")
...
func createPreset(ctx context.Context, cmd *cobra.Command, client *client.Client, name string) error {
	...
	spec := &specs.InstallationMediaConfigSpec{
		...
		Bootloader:        download.ParseBootloader(createCmdFlags.bootloader),
	}
	...
	if createCmdFlags.platform != "" {
		// Validate the platform exists before saving.
		if _, err = download.ResolveCloudPlatform(ctx, client, createCmdFlags.platform); err != nil {
			return err
		}
	...
	if createCmdFlags.overlay != "" {
		// Validate the overlay exists before saving.
		if _, err = download.ResolveOverlay(ctx, client, createCmdFlags.overlay, createCmdFlags.overlayOptions); err != nil {
			return err
		}
