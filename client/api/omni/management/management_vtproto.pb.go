...
func (this *RebootMachineResponse) EqualVT(that *RebootMachineResponse) bool {
	...
	if this.RebootId != that.RebootId {
		return false
	}
...
func (m *RebootMachineResponse) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.RebootId) > 0 {
		i -= len(m.RebootId)
		copy(dAtA[i:], m.RebootId)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.RebootId)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
...
func (m *RebootMachineResponse) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field RebootId", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.RebootId = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 3:
			if wireType != 2 {
