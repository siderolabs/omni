...
func (m *KubernetesVersionSpec) CloneVT() *KubernetesVersionSpec {
	...
	r.Version = m.Version
	r.SignatureValid = m.SignatureValid
	if rhs := m.ComponentImages; rhs != nil {
		tmpContainer := make(map[string]string, len(rhs))
		for k, v := range rhs {
			tmpContainer[k] = v
		}
		r.ComponentImages = tmpContainer
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
...
func (this *KubernetesVersionSpec) EqualVT(that *KubernetesVersionSpec) bool {
	...
	if len(this.ComponentImages) != len(that.ComponentImages) {
		return false
	}
	for i, vx := range this.ComponentImages {
		vy, ok := that.ComponentImages[i]
		if !ok {
			return false
		}
		if vx != vy {
	...
	if this.SignatureValid != that.SignatureValid {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}
...
func (m *KubernetesVersionSpec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.ComponentImages) > 0 {
		for k := range m.ComponentImages {
			v := m.ComponentImages[k]
			baseI := i
			i -= len(v)
			copy(dAtA[i:], v)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(v)))
...
func (m *KubernetesVersionSpec) SizeVT() (n int) {
	...
	if len(m.ComponentImages) > 0 {
		for k, v := range m.ComponentImages {
			_ = k
			_ = v
			mapEntrySize := 1 + len(k) + protohelpers.SizeOfVarint(uint64(len(k))) + 1 + len(v) + protohelpers.SizeOfVarint(uint64(len(v)))
			n += mapEntrySize + 1 + protohelpers.SizeOfVarint(uint64(mapEntrySize))
...
func (m *KubernetesVersionSpec) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ComponentImages", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.ComponentImages == nil {
				m.ComponentImages = make(map[string]string)
			}
			var mapkey string
			var mapvalue string
			for iNdEx < postIndex {
			...
			m.ComponentImages[mapkey] = mapvalue
			iNdEx = postIndex
		case 3:
			if wireType != 0 {
