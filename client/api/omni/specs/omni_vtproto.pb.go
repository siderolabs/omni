...
func (m *InstallationMediaConfigSpec_SBC) CloneVT() *InstallationMediaConfigSpec_SBC {
	...
	r.Overlay = m.Overlay
	if rhs := m.OverlayOptions; rhs != nil {
		tmpContainer := make(map[string]string, len(rhs))
		for k, v := range rhs {
			tmpContainer[k] = v
		}
		r.OverlayOptions = tmpContainer
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
...
func (this *InstallationMediaConfigSpec_SBC) EqualVT(that *InstallationMediaConfigSpec_SBC) bool {
	...
	if len(this.OverlayOptions) != len(that.OverlayOptions) {
		return false
	}
	for i, vx := range this.OverlayOptions {
		vy, ok := that.OverlayOptions[i]
		if !ok {
			return false
		}
		if vx != vy {
...
func (m *InstallationMediaConfigSpec_SBC) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.OverlayOptions) > 0 {
		for k := range m.OverlayOptions {
			v := m.OverlayOptions[k]
			baseI := i
			i -= len(v)
			copy(dAtA[i:], v)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(v)))
...
func (m *InstallationMediaConfigSpec_SBC) SizeVT() (n int) {
	...
	if len(m.OverlayOptions) > 0 {
		for k, v := range m.OverlayOptions {
			_ = k
			_ = v
			mapEntrySize := 1 + len(k) + protohelpers.SizeOfVarint(uint64(len(k))) + 1 + len(v) + protohelpers.SizeOfVarint(uint64(len(v)))
			n += mapEntrySize + 1 + protohelpers.SizeOfVarint(uint64(mapEntrySize))
...
func (m *InstallationMediaConfigSpec_SBC) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			if m.OverlayOptions == nil {
				m.OverlayOptions = make(map[string]string)
			}
			var mapkey string
			var mapvalue string
			for iNdEx < postIndex {
			...
			m.OverlayOptions[mapkey] = mapvalue
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
	...
	for iNdEx < l {
		...
		case 11:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.Bootloader |= InstallationMediaConfigSpec_Bootloader(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
