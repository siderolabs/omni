...
func (m *ClusterSecretsSpec) CloneVT() *ClusterSecretsSpec {
	...
	r.RotateTalosCaVersion = m.RotateTalosCaVersion
	if rhs := m.Data; rhs != nil {
		tmpBytes := make([]byte, len(rhs))
		copy(tmpBytes, rhs)
		r.Data = tmpBytes
...
func (this *ClusterSecretsSpec) EqualVT(that *ClusterSecretsSpec) bool {
	...
	if this.RotateTalosCaVersion != that.RotateTalosCaVersion {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}
...
func (m *ClusterSecretsSpec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.RotateTalosCaVersion) > 0 {
		i -= len(m.RotateTalosCaVersion)
		copy(dAtA[i:], m.RotateTalosCaVersion)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.RotateTalosCaVersion)))
		i--
		dAtA[i] = 0x32
	}
...
func (m *ClusterSecretsSpec) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 6:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field RotateTalosCaVersion", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.RotateTalosCaVersion = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
