...
#
# Generated on 2026-08-17T19:32:33Z by kres 72c98c6.
...
JS_TOOLCHAIN ?= docker.io/node:24.18.1-alpine
TOOLCHAIN ?= docker.io/golang:1.26.6-alpine
...
.PHONY: sign-image-omni-integration-test
sign-image-omni-integration-test:  ## Signs the image for omni-integration-test. Requires interactive Google authentication.
	@test -n "$$GITHUB_TOKEN" || { \
	  echo 'GITHUB_TOKEN with write:packages scope must be set: gh auth refresh -s write:packages, then GITHUB_TOKEN=$$(gh auth token) make' $@; \
	  exit 1; \
	}
