...
![Version: 2.0.0](https://img.shields.io/badge/Version-2.0.0-informational?style=flat) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat) ![AppVersion: v1.5.0](https://img.shields.io/badge/AppVersion-v1.5.0-informational?style=flat)
...
```bash
helm install omni ./deploy/helm/v2/omni -n omni --create-namespace -f values.yaml
```

To upgrade an existing installation:

```bash
helm upgrade omni ./deploy/helm/v2/omni -n omni -f values.yaml
```
...
| `omni-k8s.example.com`         | Kubernetes proxy       |
...
# Required annotation for Traefik to handle gRPC traffic
service:
  grpc:
    annotations:
...
| config.account.id | string | `""` | Unique identifier and name for this Omni instance. Must be a UUID and not changed during the lifetime of the installation. |
...
| config.services.siderolink.wireGuard.advertisedEndpoint | string | `""` | The advertised address for WireGuard connections. MUST be specified in IP:PORT format (hostname is NOT supported, must be an IP address). In most setups, this is either: - NodePort: Use the externally accessible IP of a node and service.wireguard.nodePort (e.g., "203.0.113.10:30180") - LoadBalancer: Use the LoadBalancer's external IP and service.wireguard.port (e.g., "203.0.113.50:50180") |
...
| gatewayApi.grpc.annotations | object | `{}` | Additional Annotations |
| gatewayApi.grpc.enabled | bool | `false` | Enable GRPCRoute for gRPC API. |
| gatewayApi.grpc.hostnames | list | `["omni.example.com"]` | Omni GRPC hostname |
| gatewayApi.grpc.labels | object | `{}` | Additional Labels |
| gatewayApi.grpc.parentRefs | list | `[]` | The Gateway(s) to attach this route to. You MUST define at least one parentRef for the route to be active. |
...
| ingress.grpc.annotations | object | `{}` | Additional Annotations Example annotations for NGINX Ingress: nginx.ingress.kubernetes.io/backend-protocol: GRPC nginx.ingress.kubernetes.io/proxy-body-size: 32m nginx.ingress.kubernetes.io/service-upstream: "true" |
| ingress.grpc.className | string | `""` | Ingress Class Name |
| ingress.grpc.enabled | bool | `false` | Enable ingress for gRPC API. |
| ingress.grpc.host | string | `"omni.example.com"` | Omni GRPC hostname |
| ingress.grpc.labels | object | `{}` | Additional Labels |
| ingress.grpc.skipConfigCheck | bool | `false` | Set to true to skip validation between this Ingress and config.services.api.advertisedURL |
| ingress.grpc.tls | list | `[]` | TLS configuration |
...
| ingress.main.annotations | object | `{}` | Additional Annotations |
| ingress.main.className | string | `""` | Ingress Class Name |
| ingress.main.enabled | bool | `false` | Enable ingress for Web UI. |
| ingress.main.host | string | `"omni.example.com"` | Omni UI hostname |
...
| ingress.siderolinkApi.annotations | object | `{}` | Additional Annotations Example annotations for NGINX Ingress: nginx.ingress.kubernetes.io/backend-protocol: GRPC nginx.ingress.kubernetes.io/proxy-body-size: 100m |
...
| service.grpc.clusterIP | string | `""` | ClusterIP for the gRPC service. |
| service.grpc.omniPort | int | `8080` | Port for the Omni gRPC API. |
| service.grpc.siderolinkApiPort | int | `8090` | Port for the SideroLink API. |
| service.grpc.type | string | `"ClusterIP"` | Service type for the gRPC service. |
| service.main.annotations | object | `{}` | Annotations for the main service. |
| service.main.clusterIP | string | `""` | ClusterIP for the main service. |
| service.main.k8sProxyPort | int | `8095` | Port for the Kubernetes proxy. |
| service.main.omniPort | int | `8080` | Port for the Omni HTTP API and UI. |
