...
{{- define "omni.validateValues" -}}
  {{- if empty .Values.config.account.id -}}
    {{- fail "You must provide a unique UUID value for config.account.id!" -}}
  {{- end -}}

  {{- if empty .Values.config.services.siderolink.wireGuard.advertisedEndpoint -}}
    {{- fail "WireGuard advertised endpoint must be provided in the IP:PORT format" -}}
  {{- end -}}

  {{- if empty .Values.config.auth.initialUsers -}}
    {{- fail "At least one initial user must be provided in config.auth.initialUsers" -}}
  {{- end -}}

{{/* Etcd Encryption Key Check */}}
  {{- if not .Values.etcdEncryptionKey.existingSecret -}}
    {{- if empty .Values.etcdEncryptionKey.data.omniAsc -}}
      {{- fail "You must provide an etcd encryption key in etcdEncryptionKey.data.omniAsc or use an existing secret via etcdEncryptionKey.existingSecret" -}}
    {{- end -}}
  {{- end -}}
...
{{/* Auth Provider Check - at least one must be enabled */}}
  ...
  {{- if not (or $auth0Enabled $oidcEnabled $samlEnabled) -}}
    {{- fail "At least one authentication provider must be enabled (config.auth.auth0.enabled, config.auth.oidc.enabled, or config.auth.saml.enabled)" -}}
  {{- end -}}
...
{{/* Example.com Check - prevent users from deploying with placeholder values */}}
  {{- if contains "example.com" .Values.config.services.api.advertisedURL -}}
    {{- fail "config.services.api.advertisedURL contains 'example.com'. Please set your actual domain." -}}
  {{- end -}}
  {{- if contains "example.com" .Values.config.services.kubernetesProxy.advertisedURL -}}
    {{- fail "config.services.kubernetesProxy.advertisedURL contains 'example.com'. Please set your actual domain." -}}
  {{- end -}}
  {{- if contains "example.com" .Values.config.services.machineAPI.advertisedURL -}}
    {{- fail "config.services.machineAPI.advertisedURL contains 'example.com'. Please set your actual domain." -}}
  {{- end -}}
